...
class FestMessage(Message, SenderInterface):
    ...
    def _get_data(self) -> dict[str, dict[Any, Any]]:
        temp = {"len": self.__length, "classname": "FestMessage"}
        for foo in self.__children:
            print(foo)
            temp[foo.get_id()] = self.__children[foo.get_id()].get_data()
        #for i in range(self.__length):
        #    temp[str(i)] = self.__children[i].get_data()
        return temp
